<x-layout>
    ...
    <section class="flex flex-col md:flex-row gap-4">
        <div class="bg-white p-8 rounded-lg shadow-md w-full">
            <h3 class="text-3xl text-center ront-bold mb-4">
                Profile info
            </h3>

            @if ($user->avatar)
                <div class="mt-2 flex justify-center">
                   <img src="{{asset('storage/'.$user->avatar)}}" alt="{{$user->name}}" class="
            ...
            <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                ...
                <x-inputs.text id="email" name="email" label="Email Address" 
                ...
                py-2 border rounded focus:outline-none">Save
                </button>
            </form>
        </div>
        {{-- Job listings --}}
        <div class="bg-white p-8 rounded-lg shadow-md w-full">
            ...
            @forelse ($jobs as $job)
                <div class="flex justify-between items-center border-b-2 border-gray-200 py-2">
                    ...
                    <div class="flex -space-x-3">
                        <a href="{{ route('jobs.edit', $job->id) }}"
                            class="bg-blue-500 
                    text-white px-4 py-2 rounded text-sm">Edit</a>
                        <!-- Delete Form -->
                        <form method="POST" action="{{ route('jobs.destroy', $job->id) }}?from=dashboard"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="px-4 ml-5 py-2 bg-red-500 hover:bg-red-600 text-white rounded
                                         text-sm">
                                Delete
                            </button>
                        </form>
                ...
                Applicants
                    ...
                    @empty
                        <p class="text-gray-700 mb-4"> No applicants 
                            for this job </p>
                    @endforelse
                </div>
            @empty
