<x-layout>
    ...
    <div x-cloak x-show="open" 
         class="fixed inset-0 flex items-center justify-center bg-gray-900/30 backdrop-blur-sm p-4">
        <div @click.away="open = false"
             class="bg-white p-6 rounded-lg shadow-md w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" action="{{route('applicant.store', $job->id)}}" enctype="multipart/form-data">
                ...
                <x-inputs.text id="contact_email" name="contact_email" label="Contact Email" type="email" :required="true"/>
...
@else 
   <p class="my-5 bg-gray-200 rounded-xl p-3"> <i class="fas fa-info-circle mr-3"></i> You must logged in to apply for this job</p> 
@endauth
                 ...
                 <aside class="bg-white rounded-lg shadow-md p-3">
                    ...
                    @guest
                    <p class="mt-10 bg-gray-200 text-gray-700 font-bold w-full py-2 px-4
                     rounded-full text-center">
                     <i class="fas fa-info-circle mr-3"></i> You must be logged in to 
                     bookmark a job
                    </p>

                    @else    

                    <form method="POST" action="{{auth()->user()->bookmarkedJobs()->where('job_id',
                    $job->id)->exists() ? route('bookmarks.destroy', $job->id) : route('bookmarks.store', 
                    $job->id )}}" 
                        class="mt-10">
                     @csrf
                     ...
                     <button class="bg-red-500 hover:bg-red-600 text-white
                      font-bold w-full p-1 rounded-full flex items-center justify-center">
                     <i class="fas fa-bookmark mr-3"></i>Remove Bookmark
                    </button>
 
                     @else
                    <button class="bg-blue-500 hover:bg-blue-600 text-white
                      font-bold w-full px-4 rounded-full flex items-center justify-center">
